...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $stmtUserFull = $db->prepare("SELECT firstname, lastname, email FROM users WHERE id = ?");
        $stmtUserFull->execute([$userId]);
        $userInfos = $stmtUserFull->fetch(PDO::FETCH_ASSOC);

        $realLastname  = Security::decrypt($userInfos['lastname']);
        $realFirstname = Security::decrypt($userInfos['firstname']);
        
        // Construction du nom pour la facture (Ex: DUPONT Jean)
        // Si vide (cas rare), on garde un fallback
        if (!empty($realLastname) || !empty($realFirstname)) {
            $nomClientFacture = mb_strtoupper($realLastname) . " " . ucfirst($realFirstname);
        } else {
            $nomClientFacture = "Client Web " . $username; // Fallback
        }
        ...
        if (!$clientRow) {
            ...
            $stmtFix = $db->prepare("INSERT INTO client (code_client, user_id, nom, email_fact) VALUES (?, ?, ?, ?)");
            $emailFact = $_POST['email'] ?? 'unknown@email.com';
            $stmtFix->execute([$codeClient, $userId, $nomClientFacture, $emailFact]);
        }else {
            $codeClient = $clientRow['code_client'];
            // Update the name on the client record to ensure it's current
            $db->prepare("UPDATE client SET nom = ? WHERE code_client = ?")->execute([$nomClientFacture, $codeClient]);
        }
